...
Current execution support is intentionally narrow: explicit `danger-full-access` runs as local, non-sandboxed execution. On Windows, sandboxed policies such as `read-only`, `workspace-contained`, and `workspace-write` execute through the reference backend. On Linux, `read-only` may be reported and executed as experimental when the bubblewrap probe gate is available. Other sandboxed policies on non-Windows platforms still fail closed until a backend can enforce them.
...
On macOS and Linux, RunSeal reports explicit experimental/community backends. macOS currently supports only explicit `danger-full-access` local execution. Linux also has an experimental bubblewrap `read-only` path when the runtime probe gate passes; `workspace-write`, `workspace-contained`, and network enforcement remain unsupported until they pass shared conformance gates.
