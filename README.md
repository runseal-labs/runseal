...
On macOS, RunSeal reports an explicit experimental skeleton backend that fails closed for sandboxed policies. On Linux, RunSeal reports experimental `read-only` and `workspace-write` paths while leaving other sandbox levels unsupported. Portable capability probes are diagnostic only and do not promote unsupported capabilities.
...
The portable smoke checks diagnostic capability probes and structured
fail-closed behavior for sandboxed policies. It does not promote macOS or Linux
sandbox support.
