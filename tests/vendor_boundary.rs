...
#[test]
fn protocol_modules_do_not_depend_on_command_modules() {
    for entry in std::fs::read_dir(RUNSEAL_PROTOCOL_DIR).unwrap() {
        let path = entry.unwrap().path();
        if path.extension().and_then(|extension| extension.to_str()) != Some("rs") {
            continue;
...
#[test]
fn service_modules_do_not_depend_on_protocol_modules() {
    for entry in std::fs::read_dir(RUNSEAL_SERVICE_DIR).unwrap() {
        let path = entry.unwrap().path();
        if path.extension().and_then(|extension| extension.to_str()) != Some("rs") {
            continue;
        ...
        let source = std::fs::read_to_string(&path).unwrap();
        for forbidden in ["crate::protocol", "super::protocol"] {
            assert!(
                !source.contains(forbidden),
                "{} must not depend on protocol module token {forbidden}",
                path.display()
            );
        }
    }
