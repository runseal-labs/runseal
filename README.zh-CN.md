...
当前执行能力刻意保持窄边界：显式 `danger-full-access` 会作为本地非沙箱执行运行。Windows 上，`read-only`、`workspace-contained`、`workspace-write` 等沙箱策略通过 reference backend 执行。macOS 上，`read-only` 和 `workspace-write` 搭配 `network.disabled` 处于 experimental 状态。Linux 上，`read-only`、`workspace-write` 和 `workspace-contained` 搭配 `network.disabled` 处于 experimental 状态，并在 runtime guard 可用时通过 portable backend 执行。其他 portable sandbox 策略在能强制执行前仍 fail closed。

macOS 当前报告 experimental `read-only` 和 `workspace-write` 路径，其他 sandbox level 仍 unsupported。Linux 当前报告 experimental `read-only`、`workspace-write` 和 `workspace-contained` 路径。Portable capability probe 仅用于诊断，不会提升 unsupported capability。
