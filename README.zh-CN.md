...
### macOS 和 Linux parity evidence

Windows 是一等公民的 reference backend。下面的 macOS 和 Linux 项是给社区贡献者拆分的工作项：只有对应平台通过列出的 conformance evidence 后，capability 才应向 `supported` 推进。

| Area | Windows reference | macOS experimental | Linux experimental | Evidence needed for promotion |
| --- | --- | --- | --- | --- |
| Filesystem levels | `read-only`、`workspace-write` 和 `workspace-contained` supported | `read-only` 和 `workspace-write` experimental；`workspace-contained` unsupported | `read-only` 和 `workspace-write` experimental；`workspace-contained` unsupported | 共享 filesystem conformance，加上 adversarial external read/write、parent traversal、symlink 或 junction traversal、protected metadata 和 runtime-root cases。 |
