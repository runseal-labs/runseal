...
当前执行能力刻意保持窄边界：`danger-full-access` 以本地非沙箱方式执行。`read-only`、`workspace-write` 和 `workspace-contained` 在 Windows、macOS、Linux 三个平台都 supported。Windows 仍是完整 reference backend；macOS 也通过 experimental backend 支持 `network.proxy`。macOS 和 Linux 通过默认拒绝的最小平台视图实现 host-read containment。
...
macOS 和 Linux 上，RunSeal 支持 `read-only`、`workspace-write` 和 `workspace-contained`，默认网络语义是 unmanaged 直通。`workspace-contained` 只暴露 workspace、私有 runtime roots、显式 policy read roots 和最小只读系统执行基线，其他 host 路径不可读。需要拒绝网络时可以显式请求 `network.disabled`。macOS 还支持 experimental `network.proxy`：native sandbox 只允许连接当前 execution 的 managed proxy endpoint，direct external 和无关 loopback 连接仍被拒绝。Linux `network.proxy` 仍为 unsupported。
...
| Network modes | `network.unmanaged`、`network.disabled` 和 `network.proxy` supported | experimental backend 支持 `network.unmanaged`、`network.disabled` 和 `network.proxy` | `network.unmanaged` 和 `network.disabled` supported；`network.proxy` unsupported | `network.unmanaged` 的 direct pass-through 行为；`network.disabled` 的 direct socket 和 HTTP egress denial；`network.proxy` 的 managed proxy routing 和 `CONNECT` tunneling、environment override resistance、direct 和无关 loopback bypass denial、credential redaction、audit/event coverage，以及 public-safe fail-closed output。 |
