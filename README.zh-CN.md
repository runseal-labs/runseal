...
当前执行能力刻意保持窄边界：显式 `danger-full-access` 会作为本地非沙箱执行运行。Windows 上，`read-only`、`workspace-contained`、`workspace-write` 等沙箱策略通过 reference backend 执行。Linux 上，当 bubblewrap probe gate 可用时，`read-only` 可报告并执行为 experimental。其他非 Windows 沙箱策略在能强制执行前仍会 fail closed。

macOS 和 Linux 当前报告 explicit experimental/community backend。macOS 目前只支持显式 `danger-full-access` 本地执行。Linux 还包含一个 probe-gated experimental bubblewrap `read-only` 路径；`workspace-write`、`workspace-contained` 和 network enforcement 仍需通过 shared conformance gates 后才能声明支持。
...
- Escape 和 adversarial conformance model：https://github.com/runseal-labs/rfcs/blob/main/rfcs/0015-escape-definition-and-adversarial-conformance.md
- Adversarial conformance harness 和 case format：https://github.com/runseal-labs/rfcs/blob/main/rfcs/0016-adversarial-conformance-harness-and-case-format.md
