...
`0.1.2` 是当前面向第三方集成的技术预览版本。当前仓库包含可构建的 CLI/RPC shell、标准策略 profile 归一化、canonical policy hash、backend capability reporting、Windows reference backend、`PlatformSandboxPlan` 摘要、JSONL audit 输出和黑盒 conformance tests。

当前执行能力刻意保持窄边界：显式 `danger-full-access` 会作为本地非沙箱执行运行。Windows 上，`read-only`、`workspace-contained`、`workspace-write` 等沙箱策略通过 reference backend 执行。macOS 和 Linux 上，`read-only`、`workspace-write` 搭配 `network.disabled` 处于 experimental 状态。其他 portable sandbox 策略在能强制执行前仍 fail closed。

macOS 和 Linux 当前报告 experimental `read-only` 和 `workspace-write` 路径，其他 sandbox level 仍 unsupported。这些 portable 路径只声明写边界和网络边界，不声明 workspace containment；在 `workspace-contained` 独立实现并报告前，host 文件仍可能可读。portable process cleanup 也是 experimental，不等同于 Windows reference cleanup。

客户端做能力判断时应优先使用 `sandbox_levels`、`network_modes` 和 `feature_statuses`。旧的 `features` 布尔值只是粗粒度存在标记；Portable capability probe 仅用于诊断，不会提升 unsupported capability。
...
推送 `v*` tag 会运行 `.github/workflows/release.yml`，构建原生 release archives，并发布 `.sha256` 校验文件。手动触发同一个 workflow 并传入已有 tag 时，会重新构建包并覆盖该 release 的现有 assets。
...
portable smoke 会检查 diagnostic capability probe、可用 experimental portable
enforcement，以及 unsupported 沙箱策略的结构化 fail-closed 行为。它不会把
portable capability 提升为 supported。
