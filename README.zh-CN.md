...
当前执行能力刻意保持窄边界：显式 `danger-full-access` 会作为本地非沙箱执行运行。Windows 上，`read-only`、`workspace-contained`、`workspace-write` 等沙箱策略通过 reference backend 执行。非 Windows 平台上的沙箱策略在 backend 能强制执行前仍会 fail closed。

macOS 和 Linux 当前报告 explicit experimental/community backend。macOS 和 Linux 目前只支持显式 `danger-full-access` 本地执行；sandboxed filesystem 和 network enforcement 仍需通过 shared conformance gates 后才能声明支持。
